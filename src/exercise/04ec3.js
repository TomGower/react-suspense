...
function PokemonCacheProvider({children, cacheTime}) {
  ...
  */
  ...
  const getPokemonResource = React.useCallback(name => {
    ...
    let resource = cache.current[lowerName]?.resource
    const currentTime = Date.now()
    if (!resource || currentTime - cache.current[lowerName].time > cacheTime) {
      resource = createPokemonResource(lowerName)
      cache.current[lowerName] = { resource, time: currentTime }
    }
    return resource
  }, [cacheTime])
